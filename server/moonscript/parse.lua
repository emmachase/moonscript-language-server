...
local trim, pos_to_line, pos_to_line_column, get_line
do
  local _obj_0 = require("moonscript.util")
  trim, pos_to_line, pos_to_line_column, get_line = _obj_0.trim, _obj_0.pos_to_line, _obj_0.pos_to_line_column, _obj_0.get_line
end
local unpack
...
local build_grammar = wrap_env(debug_grammar, function(root)
  ...
  local g = P({
    ...
    KeyValue = (sym(":") * -SomeSpace * lpeg.Cp() * Name) / self_assign + Ct((KeyName + sym("[") * Exp * sym("]") + Space * DoubleString + Space * SingleString) * symx(":") * (Exp + TableBlock + SpaceBreak ^ 1 * Exp)),
...
file_parser = function()
  ...
  return {
    ...
    match_raw = function(self, str)
      local tree, err, errpos
      local _
      _, err = xpcall((function()
        tree, err, errpos = file_grammar:match(str)
      end), function(err)
        return debug.traceback(err, 2)
      end)
      if type(err) == "string" then
      ...
      if not (tree) then
        local line, col = re.calcline(str, errpos)
        return nil, line, col, errpos
      end
      return tree
    end
  }
...
return {
  ...
  string_raw = function(str)
    return file_parser():match_raw(str)
  end
}
